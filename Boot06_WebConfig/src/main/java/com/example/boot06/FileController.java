...
public class FileController {
    ...
    public ResponseEntity<InputStreamResource> download(String orgFileName, String saveFileName, long fileSize) throws FileNotFoundException, UnsupportedEncodingException {
        // 원래는 DB 에서 읽어와야 하지만 지금은 다운로드해 줄 파일의 정보가 요청파라미터로 전달된다.

        // 다운로드 시켜줄 원본 파일명
        String encodedName = URLEncoder.encode(orgFileName, "utf-8");
        // 파일명에 공백이 있는 경우 파일명이 이상해지는 것을 방지
        encodedName = encodedName.replace("\\+", " ");
        // 응답 헤더정보 (스프링 프레임워크에서 제공해주는 클래스) 구성하기(웹브라우저에 알림정보)
        HttpHeaders headers = new HttpHeaders();
        // 파일을 다운로드 시켜주겠다는 정보
        headers.add(HttpHeaders.CONTENT_TYPE, "application/octet-stream");
        // 파일의 이름 정보(웹 브라우저가 해당정보를 이용해서 파일을 만들어준다)
    ...
    public String upload(FileDto dto, Model model) throws IOException {
        // dto 에 있는 정보를 이용해서 C:/acorn202304/upload.jsp 폴더에 업로드된 파일을 저장해보세요
        // 파일명은 UUID + 원본파일명으로 만들어주세요.
